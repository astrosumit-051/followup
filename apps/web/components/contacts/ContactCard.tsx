...
export function ContactCard({ contact }: ContactCardProps) {
  ...
  return (
    <Link href={`/contacts/${contact.id}`} data-testid={`contact-card-${contact.id}`}>
      <Card className="transition-shadow hover:shadow-md">
        <CardHeader className="p-4 sm:p-6">
          <div className="flex flex-col space-y-3 mb-4
            ...
            <div className="flex items-center space-x-3 flex-1 min-w-0
              ...
              <div className="flex-1 min-w-0">
                ...
                {contact.company && (
                  <p className="text-sm text-muted-foreground line-clamp-1">{contact.company}</p>
                )}
              </div>
            </div>

            {/* Priority Badge */}
            <Badge variant={priorityVariant} className="self-start flex-shrink-0 sm:ml-2">
              {contact.priority}
            </Badge>
          </div>
          ...
          <div className="flex flex-col space-y-1 text-xs text-muted-foreground pt-3 border-t
