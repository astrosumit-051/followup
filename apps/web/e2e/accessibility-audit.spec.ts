...
test.describe('Accessibility Audit - WCAG 2.1 AA Compliance', () => {
  ...
  test('Dashboard (authenticated) should have no accessibility violations', async ({ page }) => {
  ...
  test('Contact list page should have no accessibility violations', async ({ page }) => {
    await page.goto('/contacts');

    // Wait for contacts to load
    await page.waitForSelector('[data-testid="contact-card"], [data-testid="empty-state"]', { timeout: 10000 });
  ...
  test('Contact creation form should have no accessibility violations', async ({ page }) => {
  ...
  test.describe('Keyboard Navigation', () => {
    ...
    test('Contact form should be fully navigable by keyboard', async ({ page }) => {
      await page.goto('/contacts/new');
    ...
    test('Theme toggle should be keyboard accessible', async ({ page }) => {
      ...
      const themeToggle = page.locator('[aria-label*="theme" i], button:has-text("Light"), button:has-text("Dark")').first();
  ...
  test.describe('ARIA Attributes', () => {
    ...
    test('Error messages should have aria-describedby', async ({ page }) => {
      ...
      await page.waitForSelector('[class*="destructive"]', { timeout: 2000 }).catch(() => {});
    ...
    test('Dialogs should have aria-labelledby and aria-describedby', async ({ page }) => {
      await page.goto('/contacts');
  ...
  test.describe('Color Contrast', () => {
    test('All text should meet WCAG AA contrast requirements (light mode)', async ({ page }) => {
      // Ensure light mode
    ...
    test('All text should meet WCAG AA contrast requirements (dark mode)', async ({ page }) => {
      // Ensure dark mode
      await page.emulateMedia({ colorScheme: 'dark' });
      await page.goto('/dashboard');

      // Toggle to dark mode via theme toggle
      const themeToggle = page.locator('[aria-label*="theme" i], button:has-text("Dark")').first();
      if (await themeToggle.isVisible()) {
  ...
  test.describe('Focus Indicators', () => {
    ...
    test('Buttons should have visible focus indicators', async ({ page }) => {
      ...
      const createButton = page.locator('button:has-text("Create Contact"), a:has-text("Create Contact")').first();
      ...
      expect(classes.includes('focus:') || classes.includes('focus-visible:')).toBeTruthy();
    });
  });
