...
test.describe('OAuth Callback Route Integration', () => {
  test.describe('Valid OAuth Code Exchange', () => {
    test('should redirect to home page after successful OAuth code exchange', async ({ page }) => {
      ...
      const isValidDestination = validDestinations.some(dest =>
        finalUrl === dest || finalUrl.includes(dest)
      );

      expect(isValidDestination).toBeTruthy();
    });

    test('should redirect to specified "next" parameter after successful OAuth', async ({ page }) => {
      // Navigate to callback with OAuth code and next parameter
      await page.goto('/auth/callback?code=valid-test-oauth-code&next=/dashboard/contacts');
    ...
    test('should sanitize and validate "next" parameter to prevent open redirects', async ({ page }) => {
      // Attempt redirect to external URL (should be prevented)
      await page.goto('/auth/callback?code=valid-test-oauth-code&next=https://evil.com/phishing');
    ...
    test('should handle "next" parameter with query strings', async ({ page }) => {
      // Navigate with complex next parameter including query strings
      await page.goto('/auth/callback?code=valid-test-oauth-code&next=/dashboard?tab=contacts&sort=name');
  ...
  test.describe('Missing OAuth Code Handling', () => {
    test('should redirect to login when OAuth code is missing', async ({ page }) => {
    ...
    test('should redirect to login when code parameter is empty', async ({ page }) => {
    ...
    test('should not include error parameter when code is simply missing', async ({ page }) => {
  ...
  test.describe('Redirect URL Security', () => {
    test('should prevent redirect to external domains', async ({ page }) => {
      ...
      for (const externalUrl of externalUrls) {
        await page.goto(`/auth/callback?code=test-code&next=${encodeURIComponent(externalUrl)}`);
    ...
    test('should allow internal path redirects', async ({ page }) => {
      ...
      for (const path of internalPaths) {
        await page.goto(`/auth/callback?code=test-code&next=${encodeURIComponent(path)}`);
    ...
    test('should handle path traversal attempts in next parameter', async ({ page }) => {
      ...
      for (const attempt of traversalAttempts) {
        await page.goto(`/auth/callback?code=test-code&next=${encodeURIComponent(attempt)}`);
    ...
    test('should default to home page for invalid next parameter', async ({ page }) => {
      ...
      for (const invalidParam of invalidNextParams) {
        await page.goto(`/auth/callback?code=test-code&next=${encodeURIComponent(invalidParam)}`);
  ...
  test.describe('Error Handling and Recovery', () => {
    test('should log errors without exposing sensitive information', async ({ page, context }) => {
  ...
  test.describe('Edge Cases', () => {
    test('should handle multiple concurrent callback requests', async ({ page, context }) => {
    ...
    test('should handle very long next parameter', async ({ page }) => {
      ...
      await page.goto(`/auth/callback?code=test-code&next=${encodeURIComponent(longPath)}`);
    ...
    test('should handle special characters in next parameter', async ({ page }) => {
      const specialChars = '/dashboard?name=Test User&email=test@example.com&tags=tag1,tag2';

      await page.goto(`/auth/callback?code=test-code&next=${encodeURIComponent(specialChars)}`);
