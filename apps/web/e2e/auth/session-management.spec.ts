...
test.describe('Session Persistence and Management', () => {
  test.describe('Session Persistence Across Page Refreshes', () => {
    ...
    test('should maintain session across multiple page navigations', async ({ page, context }) => {
      // Simulate authenticated session
      await context.addCookies([
        {
          name: 'sb-localhost-auth-token',
    ...
    test('should persist session in background tabs', async ({ page, context }) => {
      // Simulate authenticated session
      await context.addCookies([
        {
          name: 'sb-localhost-auth-token',
  ...
  test.describe('Automatic Token Refresh', () => {
    test('should automatically refresh expiring session tokens', async ({ page, context }) => {
    ...
    test('should handle token refresh failures gracefully', async ({ page, context }) => {
    ...
    test('should refresh token during long user sessions', async ({ page, context }) => {
      ...
      const routes = ['/dashboard', '/contacts', '/settings', '/profile', '/dashboard'];
  ...
  test.describe('Session Expiration Handling', () => {
    ...
    test('should handle expired session during active use', async ({ page, context }) => {
  ...
  test.describe('HttpOnly Cookie Security', () => {
    test('should prevent JavaScript access to session cookies', async ({ page, context }) => {
      // Set httpOnly cookie
      await context.addCookies([
        {
          name: 'sb-localhost-auth-token',
  ...
  test.describe('Logout Functionality', () => {
    test('should clear session when user logs out', async ({ page, context }) => {
      // Simulate authenticated session
      await context.addCookies([
        {
          name: 'sb-localhost-auth-token',
      ...
      if (await logoutButton.isVisible({ timeout: 1000 })) {
        ...
        const url = page.url();
        const loggedOut = url.includes('/login') || url === 'http://localhost:3000/' || url === 'http://localhost:3000';
    ...
    test('should log out from all tabs simultaneously', async ({ page, context }) => {
      // Simulate authenticated session
      await context.addCookies([
        {
          name: 'sb-localhost-auth-token',
    ...
    test('should handle logout errors gracefully', async ({ page, context }) => {
      ...
      if (await logoutButton.isVisible({ timeout: 1000 })) {
        ...
        const url = page.url();
        const redirectedSafely = url.includes('/login') || url === 'http://localhost:3000/' || url === 'http://localhost:3000';
  ...
  test.describe('Concurrent Session Management', () => {
    test('should handle multiple concurrent requests with same session', async ({ page, context }) => {
      // Simulate authenticated session
      await context.addCookies([
        {
          name: 'sb-localhost-auth-token',
      ...
      const url = page.url();
      const handled = url.includes('/dashboard') || url.includes('/contacts') || url.includes('/settings') || url.includes('/login');

      expect(handled).toBeTruthy();
    });

    test('should maintain session consistency across parallel API calls', async ({ page, context }) => {
      // Simulate authenticated session
      await context.addCookies([
        {
          name: 'sb-localhost-auth-token',
