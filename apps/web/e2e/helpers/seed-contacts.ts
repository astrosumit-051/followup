...
export async function seedContacts(userSupabaseId: string): Promise<string[]> {
  console.log(
    `🌱 Seeding contacts for user with Supabase ID: ${userSupabaseId}`,
  );
  ...
  if (!user) {
    throw new Error(
      `User with supabaseId ${userSupabaseId} not found in database`,
    );
  }
  ...
  console.log(
    `🗑️  Deleted ${deletedCount.count} existing contacts for clean slate`,
  );
...
export async function clearContacts(userSupabaseId: string): Promise<number> {
  ...
  console.log(
    `🗑️  Cleared ${deletedCount.count} contacts for user ${user.email}`,
  );
