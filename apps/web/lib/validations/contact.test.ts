...
describe('Contact Validation Schemas', () => {
  describe('createContactSchema', () => {
    describe('name field', () => {
      ...
      it('should reject whitespace-only name', () => {
        ...
        if (!result.success) {
          const errorMessages = result.error.issues.map(e => e.message);
          expect(errorMessages.some(msg => msg.includes('cannot be only whitespace'))).toBe(true);
        }
      });
      ...
      it('should reject name longer than 255 characters', () => {
        ...
        if (!result.success) {
          const errorMessages = result.error.issues.map(e => e.message);
          expect(errorMessages.some(msg => msg.includes('less than 255 characters'))).toBe(true);
        }
      });
    ...
    describe('phone field', () => {
      ...
      it('should reject phone longer than 50 characters', () => {
        ...
        if (!result.success) {
          const errorMessages = result.error.issues.map(e => e.message);
          expect(errorMessages.some(msg => msg.includes('less than 50 characters'))).toBe(true);
        }
      });
    });
    ...
    describe('linkedInUrl field', () => {
      ...
      it('should reject invalid URL format', () => {
        ...
        if (!result.success) {
          const errorMessages = result.error.issues.map(e => e.message);
          expect(errorMessages.some(msg => msg.includes('Invalid URL format'))).toBe(true);
        }
      });
    });
    ...
    describe('birthday field', () => {
      ...
      it('should reject invalid date format', () => {
        ...
        if (!result.success) {
          const errorMessages = result.error.issues.map(e => e.message);
          expect(errorMessages.some(msg => msg.includes('expected date'))).toBe(true);
        }
      });
    });
    ...
    describe('notes field', () => {
      ...
      it('should reject notes longer than 10,000 characters', () => {
        ...
        if (!result.success) {
          const errorMessages = result.error.issues.map(e => e.message);
          expect(errorMessages.some(msg => msg.includes('less than 10,000 characters'))).toBe(true);
        }
      });
  ...
  describe('contactPaginationSchema', () => {
    ...
    it('should reject non-UUID cursor', () => {
      ...
      if (!result.success) {
        const errorMessages = result.error.issues.map(e => e.message);
        expect(errorMessages.some(msg => msg.includes('valid UUID'))).toBe(true);
      }
    });
    ...
    it('should reject limit greater than 100', () => {
      ...
      if (!result.success) {
        const errorMessages = result.error.issues.map(e => e.message);
        expect(errorMessages.some(msg => msg.includes('cannot exceed 100'))).toBe(true);
      }
    });
    ...
    it('should reject non-integer limit', () => {
      ...
      if (!result.success) {
        const errorMessages = result.error.issues.map(e => e.message);
        expect(errorMessages.some(msg => msg.includes('must be an integer'))).toBe(true);
      }
    });
  });
